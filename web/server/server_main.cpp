...
int main() {
    try {
        ...
        OrderBookServer server(&book, &pool, &stats, 9090);
        ...
        std::cout << "Starting WebSocket server on port 9090..." << std::endl;
        ...
        std::cerr << "\n[FATAL ERROR] Exception caught in main(): " << e.what() << std::endl;
        return 1;
    } catch (...) {
        std::cerr << "\n[FATAL ERROR] Unknown exception caught in main()" << std::endl;
        return 1;
    }
}
